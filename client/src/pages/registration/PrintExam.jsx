...
function PrintExam() {
  ...
  const examId = searchParams.get("id"); // ✅ ดึงค่า id จาก query string

  const handleSearch = async () => {
    console.log("Searching for:", thaiId);
    if (!thaiId.trim()) return setError("กรุณากรอกเลขบัตรประชาชน");

    try {
      const res = await axios.get(
        `http://localhost:5000/api/PrintExam/search/${thaiId.trim()}?id=${examId}`
      );
      setData(res.data);
      setError("");
    } catch (err) {
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
            <div className="flex flex-col md:flex-row md:gap-x-8">
              ...
              <main className="w-full md:w-[70%]">
                ...
                {data && (
                  <div className="w-full mt-[2rem] overflow-x-auto">
                    <table className="min-w-[600px] w-full border border-gray-300 rounded-[5px] overflow-hidden text-sm sm:text-base">
                      ...
                      <tbody>
                        <tr className="hover:bg-gray-50 odd:bg-gray-100 even:bg-white">
                          ...
                          <td className="px-4 py-2 border-b whitespace-nowrap">{data.firstname}</td>
                          <td className="px-4 py-2 border-b whitespace-nowrap">{data.lastname}</td>
                          <td className="px-4 py-2 border-b">
                            {data.exam_place}<br />{data.exam_building}
                          </td>
          ...
          <div className="border-2 border-gray-500 p-6 text-[1rem] text-black leading-relaxed">
            <div className="flex justify-between items-start border-b border-gray-500 pb-4 mb-4">
              ...
              <div className="w-[8rem] h-[10rem] overflow-hidden rounded-[5px]">
                <img src={Juthawut} alt="QR Code" className="h-full w-full object-cover object-top" />
              </div>
            </div>

            <div className="space-y-4">
              <div className="flex"><p className="font-semibold w-[180px]">ชื่อ-นามสกุล:</p><p>{data.firstname} {data.lastname}</p></div>
              <div className="flex"><p className="font-semibold w-[180px]">เลขบัตรประชาชน:</p><p>{data.thai_id}</p></div>
              <div className="flex"><p className="font-semibold w-[180px]">รหัสประจำตัวผู้เข้าสอบ:</p><p>{data.enrollments_id}</p></div>
              <div className="flex"><p className="font-semibold w-[180px]">สถานที่สอบ:</p><p>{data.exam_place}<br />{data.exam_building} ห้อง {data.exam_room}</p></div>
            </div>
              ...
              <table className="w-full text-sm table-fixed">
                <thead>
                  <tr className="bg-gray-200 border-2 border-gray-500">
                    <th className="px-4 py-2 border-b font-medium w-[70%]">รายการ</th>
                    <th className="px-4 py-2 border-b border-2 border-gray-500 font-medium w-[30%]">จำนวนเงิน</th>
                  </tr>
                </thead>
                <tbody>
                  <tr className="border-2 border-gray-500">
                    <td className="px-4 py-2 border-2 border-gray-500 w-[70%]">
                      ...
                      <p>
                        เวลา{" "}
                        {new Date(data.start_datetime).toLocaleTimeString("en-GB", {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}{" "}
                        น.
                      </p>
                      <p>รหัสวิชา : {data.examination_id} รายวิชา : {data.exam_set_name}</p>
                      <span className="flex">
                        ประเภทข้อสอบ
                    ...
                    <td className="px-4 py-2 border-b w-[30%] align-top">
                      <div className="flex flex-col justify-between h-full min-h-[100px]">
                        ...
                        <div className="border-t-2 border-gray-500 pt-[5px] font-semibold">
                          <p>ยอดชำระ 500 บาท</p>
                          <span className="flex">
                            สถานะ : <p className="text-red-500 ml-[5px]">ยังไม่ชำระ</p>
                          </span>
                        </div>
                      </div>
                    </td>
